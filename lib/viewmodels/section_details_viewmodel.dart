...
/// ViewModel for [SectionDetailsScreen].
/// Holds selected day, class list, loading state, and all
/// time-calculation helpers. Logic identical to original _SectionDetailsScreenState.
class SectionDetailsViewModel extends ChangeNotifier {
  ...
  SectionDetailsViewModel({required this.sectionId}) {
    _initialize();
  }
  ...
  Future<void> _loadFromCache() async {
    try {
      final cached = await _dbHelper.getTimetable(sectionId, 'student');
  ...
  Future<void> _fetchDayDataInBackground() async {
    try {
      final isFresh =
          await _dbHelper.isCacheFresh(sectionId, 'student', minutesThreshold: 5);
      if (isFresh) {
        debugPrint("Using fresh cache, skipping Firebase fetch");
        return;
      }

      final hasCache = await _dbHelper.hasCache(sectionId, 'student');
      if (!hasCache) {
        _loading = true;
        notifyListeners();
      }
      ...
      if (doc.exists) {
        final data = doc.data();
        if (data != null) {
          await _dbHelper.insertOrUpdateTimetable(sectionId, 'student', data);
